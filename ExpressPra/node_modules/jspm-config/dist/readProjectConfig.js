...
function readDependenciesJson(jspmPackageJson, options) {
    var packages = jspmPackageJson.directories ?
        jspmPackageJson.directories.packages :
        constants_1.JSPM_PACKAGE_JSON_DEFAULT.directories.packages;
    return fs_1.readJson(path.join(options.cwd, packages, '.dependencies.json')).catch(function (err) {
        if (err.code === 'ENOENT') {
            return;
        }
        throw err;
...
function readJspmConfigs(jspmPackageJson, options) {
    var baseURL = jspmPackageJson.directories ?
        jspmPackageJson.directories.baseURL :
        constants_1.JSPM_PACKAGE_JSON_DEFAULT.directories.baseURL;
